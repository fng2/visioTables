...
#Region "List Of Methods"
    ...
    Public Sub SelCell(arg As Byte) ' Выделение(разное) ячеек таблицы
        ...
        Dim intMaxC As Integer
        Dim intMaxR As Integer
        Dim NoDupes As New Collection
...
err:
    ...
    Sub SelectCells(intStartCol As Integer, intEndCol As Integer,
    intStartRow As Integer, intEndRow As Integer) 'Различное выделение в таблице
        ...
        For intColNum = intStartCol To intEndCol
            For intRowNum = intStartRow To intEndRow
                Globals.ThisAddIn.Application.ActiveWindow.Select(Globals.ThisAddIn.Application.ActiveWindow.Page.Shapes.ItemFromID(ArrShapeID(intColNum, intRowNum)), Visio.VisSelectArgs.visSelect)
            Next
        Next
        On Error GoTo 0
    End Sub
    ...
    Sub AddColumns(arg As Byte)  ' Вставка нового столбца. Основная процедура
        ...
        vs = winObj.Selection
        iAll = Convert.ToInt32(shpsObj.Item(NT).Cells(UTC))
        ...
        If arg = 0 Then   ' Вставка столбца перед выделенным
            ...
            For i = nCol To UBound(ArrShapeID, 1) ' Перенумерация управляющих ячеек
                ...
                shpsObj.ItemFromID(ArrShapeID(i, 0)).Cells(UTC).FormulaForceU = GU & Convert.ToInt32(shpsObj.ItemFromID(ArrShapeID(i, 0)).Cells(UTC)) + 1 & ")"
            Next
        ...
        ElseIf arg = 1 Then  ' Вставка столбца после выделенного
            ...
            End With
            If nCol <> Convert.ToInt32(shpsObj.Item(NT).Cells(UTC)) Then
    ...
    Sub AddRows(arg As Byte) ' Вставка новой строки. Основная процедура
        ...
        vs = winObj.Selection
        iAll = Convert.ToInt32(shpsObj.Item(NT).Cells(UTR))
        ...
        If arg = 0 Then ' Вставка строки перед выделенной
            ...
            For i = nRow To UBound(ArrShapeID, 2) ' Перенумерация управляющих ячеек
                'shpsObj.ItemFromID(ArrShapeID(0, i)).Cells(UTR).FormulaForceU = GU & shpsObj.ItemFromID(ArrShapeID(0, i)).Cells(UTR) + 1 & ")" 'old
                shpsObj.ItemFromID(ArrShapeID(0, i)).Cells(UTR).FormulaForceU = GU & Convert.ToInt32(shpsObj.ItemFromID(ArrShapeID(0, i)).Cells(UTR)) + 1 & ")"
            Next
...
errD:
    ...
    Sub ConvertInto1Shape() ' Преобразование таблицы в одну сгруппированную фигуру
        ...
        Globals.ThisAddIn.Application.ActivePage.Shapes.ItemU(NT).BoundingBox(1, dblLeft, dblBottom, dblRight, dblTop)
        ...
        With winObj
            .Selection.Group()
            .Selection.DeleteEx(Visio.VisDeleteFlags.visDeleteNormal)
        ...
        For i = 1 To visWorkCells.Count
            With visWorkCells(i)
                .DeleteSection(Visio.VisSectionIndices.visSectionUser)
                .DeleteSection(Visio.VisSectionIndices.visSectionAction)
...
err:
    ...
    Sub AlignOnSize(arg As Byte)
        ...
        'vsoSelection = winObj.Selection '  old, typo?
        vsoSel = winObj.Selection
        ...
        On Error Resume Next
        'For Each Shp In vsoSelection '  old, typo?
        For Each Shp In vsoSel
    ...
    Sub InsertText(arg) ' Вставить в ячейки текст, дату, время, комментарий, номер столбца, номер строки
        ...
        Select Case arg ' Надо поправить RecUndo
            ...
            Case 4
                ...
                arrArg = Split(txt, ",")
                If UBound(arrArg) <> 2 Then Exit Select
                txt = """" & arrArg(0) & """" & "&" & "User.TableCol" & "+" & Int(Val(arrArg(1))) & "&" & """" & arrArg(2) & """" : Call NumInsert
            Case 5
                msgComm = "Формат:" & vbCrLf & "Префикс, Смещение, Постфикс"
                txt = InputBox("Вставить номер столбца" & vbCrLf & msgComm & vbCrLf, title, ",0,")
                If txt = "" Then Exit Select
                arrArg = Split(txt, ",")
                If UBound(arrArg) <> 2 Then Exit Select
                txt = """" & arrArg(0) & """" & "&" & "User.TableRow" & "+" & Int(Val(arrArg(1))) & "&" & """" & arrArg(2) & """" : Call NumInsert
        End Select
        Call RecUndo("0")
        Exit Sub
...
NumInsert:
        For i = 1 To vsoSel.Count
            ...
            vsoSel(i).Characters.AddCustomFieldU(txt, Visio.VisFieldFormats.visFmtNumGenNoUnits)
        Next
        Return
    ...
    Sub SelInContent(arg) ' Выделение ячеек таблицы по критерию(текст, дата, значение, пустые/не пустые). Основная процедура
        ...
        winObj = Globals.ThisAddIn.Application.Application.ActiveWindow
        vsoSel = winObj.Selection
        ...
        For Each shpObj In vsoSel
            With shpObj
                'If .Cells(UTC) = 0 Or .Cells(UTR) = 0 Then Call sub1()
                If .Cells(UTC).ToString = "0" Or .Cells(UTR).ToString = "0" Then Call sub1()

                Select Case arg
                    Case 1 'Текст
                        If IsNumeric(.Characters.Text) Or
                    Trim(.Characters.Text) = "" Or
                    IsDate(.Characters.Text) Then Call Sub1
                    Case 2 'Числа
                        If Not IsNumeric(.Characters.Text) Then Call Sub1
                    Case 3 'Даты
                        If Not IsDate(.Characters.Text) Or IsNumeric(.Characters.Text) Then Call Sub1
                        '        Case 4 'Время
                        '            If Not IsDate(.Characters.Text) Or Not .Characters.Text Like "*:*:*" Then Call Sub1
                    Case 5 'Не числа
                        If IsNumeric(.Characters.Text) Or
                    Trim(.Characters.Text) = "" Then Call Sub1
                    Case 6 'Пустые
                        If Trim(.Characters.Text) <> "" Then Call Sub1
                    Case 7 'Не пустые
                        If Trim(.Characters.Text) = "" Then Call Sub1
...
Sub1: 'what is this???
        winObj.Select(shpObj, Visio.VisSelectArgs.visDeselectAll) : Return
    End Sub

    Sub GutT() ' Вырезание содержимого из выделенных ячеек таблицы
        Call CheckSelCells()
        Dim CLPB As New DataObject, txt As String = String.Empty
        Dim vsoSelection As Visio.Selection

        CLPB.SetText(fArrT(txt))
        My.Computer.Clipboard.SetText(fArrT(txt))
        'CLPB.PutInClipboard()
        'PutInClipboard' is not a member of 'DataObject'
    ...
    Sub PasteT() ' Вставка содержимого буфера обмена в ячейки таблицы
        ...
        On Error GoTo err
        CLPB = My.Computer.Clipboard.GetDataObject
...
err:
    ...
    Sub SetShapeText(ByVal txt As String, ByVal intStartCol As Integer, ByVal intEndCol As Integer,
    ByVal intStartRow As Integer, ByVal intEndRow As Integer)
        ...
        For intColNum = intStartCol To intEndCol
            For intRowNum = intStartRow To intEndRow
                Globals.ThisAddIn.Application.ActivePage.Shapes.ItemFromID(ArrShapeID(intColNum, intRowNum)).Characters.Text = txt
            Next
        Next

    End Sub

    Sub LinkToDataInShapes(intDataIndex, booInsertTableName, booTitleColumns,
intRowStartSourse, booInvisibleZero, intCountRowSourse, intCountColSourse, booFontBold)
        ...
        winObj = Globals.ThisAddIn.Application.ActiveWindow
        shpsObj = Globals.ThisAddIn.Application.ActivePage.Shapes
        vsoSel = winObj.Selection
        vsoDataRecordset = Globals.ThisAddIn.Application.ActiveDocument.DataRecordsets.Item(intDataIndex + 1)
        ...
        'FIX BUG : THERE IS NO FRMWAIT!
        'Load frmWait : frmWait.Caption = "Связать данные с фигурами..." : frmWait.Show() : frmWait.Repaint()

        Dim frmWait As dlgWait = Nothing
        If frmWait Is Nothing OrElse frmWait.IsDisposed Then
            frmWait = New dlgWait
        End If
        frmWait.Text = "Link to data shapes..."
        frmWait.Show()
        frmWait.Refresh()
        ...
        intEndCol = Convert.ToInt32(shpsObj.Item(NT).Cells(UTC))
        intEndRow = Convert.ToInt32(shpsObj.Item(NT).Cells(UTR))
        If Convert.ToInt32(shpsObj.Item(NT).Cells(UTC)) > intCountColSourse Then intEndCol = intCountColSourse
        If Convert.ToInt32(shpsObj.Item(NT).Cells(UTR)) - intRS > intCountRowSourse Then intEndRow = intCountRowSourse + intRS + 1
        ...
        If booInsertTableName Then
            With winObj
                .DeselectAll()
                .Select(shpsObj.ItemFromID(GetShapeId(1, 1)), Visio.VisSelectArgs.visSelect)
                .Select(shpsObj.ItemFromID(GetShapeId(UBound(ArrShapeID, 1), 1)), Visio.VisSelectArgs.visSelect)
                vsoSel = .Selection
        ...
        If booTitleColumns Then
            ...
            For i = 1 To UBound(ArrShapeID, 1)
                With shpsObj.ItemFromID(GetShapeId(i, 1 + intRowStart))
                    ...
                    If Convert.ToInt32(.Cells(UTC)) <= intCountColSourse Then
                        .LinkToData(vsoDataRecordset.ID, intRowStartSourse, False)
                        '.Characters.AddCustomFieldU("=" & .CellsSRC(Visio.VisSectionIndices.visSectionProp, i - 1, visCustPropsLabel).Name, visFmtNumGenNoUnits)
                        .Characters.AddCustomFieldU("=" & .CellsSRC(Visio.VisSectionIndices.visSectionProp, i - 1, Visio.VisCellIndices.visCustPropsLabel).Name, Visio.VisFieldFormats.visFmtNumGenNoUnits)
                        'If booFontBold Then .CellsSRC(3, 0, visCharacterStyle).FormulaU = visBold
                        If booFontBold Then .CellsSRC(3, 0, Visio.VisCellIndices.visCharacterStyle).FormulaU = 17.0#
                    End If
                End With
            Next
            intRowStart = intRowStart + 1
        ...
        For i = 1 To vsoSel.Count
            ...
            If Convert.ToInt32(vsoSel(i).Cells(UTR)) <= intRowStart Or Convert.ToInt32(vsoSel(i).Cells(UTC)) > intEndCol Or Convert.ToInt32(vsoSel(i).Cells(UTR)) > intEndRow Then winObj.Select(vsoSel(i), Visio.VisSelectArgs.visDeselect)
        Next
        ...
        For i = 1 To vsoSel.Count
            ...
            frmWait.lblProgressBar.Width = www1 * i
            frmWait.Refresh()
    ...
    Sub RefreshDataInShapes() ' Обновление источников внешних данных
        ...
        If Globals.ThisAddIn.Application.ActiveDocument.DataRecordsets.Count = 0 Then
            Exit Sub
        End If
        ...
        For i = 1 To Globals.ThisAddIn.Application.ActiveDocument.DataRecordsets.Count
            vsoDataRecordset = Globals.ThisAddIn.Application.ActiveDocument.DataRecordsets.Item(i)
            vsoDataRecordset.Refresh()
        Next
        Call RecUndo("0")

        MsgBox("Обновлено источников внешних данных - " & Globals.ThisAddIn.Application.ActiveDocument.DataRecordsets.Count, vbInformation, "Обновление данных")
        Exit Sub
...
ErrorHandler:
    ...
    Sub ResizeCells(bytCellsOrTable As Byte, booOnlyActiveCells As Boolean,
sngWidthCell As Single, sngHeightCell As Single, sngWidthTable As Single,
...
NotDublicate:  ' Заполнение коллекции значениями без дубликатов
    ...
    Sub AllAlignOnText(booOnWidth As Boolean, booOnHeight As Boolean, bytNothingOrAutoOrLockColumns As Byte,
    bytNothingOrAutoOrLockRows As Byte, booOnlySelectedColumns As Boolean, booOnlySelectedRow As Boolean)
        ...
        With shpsObj
            ...
            If booOnWidth And booOnlySelectedColumns Then ' Автовыравнивание только выделенных столбцов
                bytColumnOrRow = 4 : UT = UTC : Call NotDublicate
            ...
            If booOnHeight And booOnlySelectedRow Then ' Автовыравнивание только выделенных строк
                bytColumnOrRow = 5 : UT = UTR : Call NotDublicate
...
#Region "List Of Procedures"
    ...
    Private Sub IntegrateCells(vsoSel) ' Объединение выделенных ячеек в одну с сохранением содержимого. Основная процедура
        ...
        Next
        If flagCheck = False Then Call Err()
